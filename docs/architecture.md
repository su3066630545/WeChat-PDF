...
- 双处理模式由 Core 层统一判断：小型且本地能力支持的文件走本地处理，大型或复杂任务自动走 Server 层。
- 页面只调用 Core 层公开 API，不关心底层是本地算法还是云端算力。
...
- 服务端保持无状态：处理完成立即删除输入文件，结果文件下载完成后立即删除，仅保留任务日志。
...
- 双处理模式：自动判断文件大小和处理难度，小型文件本地处理，大型或复杂文件自动走云端。
- 业务解耦：所有 PDF 算法独立封装在 Core 层，页面只调用 API，不关心底层实现。
- 无状态服务：服务端不长期存储用户文件，处理完成立即删除资源，仅保留脱敏任务日志。
